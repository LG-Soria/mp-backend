...
import { processPaymentNotification } from "./webhooks.payments";
...
export async function dispatchMpEvent(body: MpWebhookBody, meta: MpWebhookMeta) {
  ...
  logger.info(
    `[MP Webhook] type=${type} action=${action} data.id=${dataId} user_id=${body.user_id}`
  );
  ...
  switch (type) {
    case "payment":
      ...
      // 👇 pasar también el body
      await processPaymentNotification(String(dataId), meta, body);
      return;
